...
function vRapoarte(){ if(!clientById(S.repClient)) S.repClient=CL[0]?CL[0].id:null; const rc=S.repClient;
  ...
  const exp='<button class="btn small ghost" title="extinde / restrânge textul" onclick="this.closest(\'.card\').querySelector(\'.copybox\').classList.toggle(\'full\')">⤢</button>';
  ...
  /* HERO — două carduri egale, acțiunile sus */
  h+='<div class="grid2 pj-rep"><div class="card">'+cardHead('Raport săptămânal','L4','<button class="btn small" onclick="copyTxt(window._repL4,\'Raport L4 copiat\')">📋 copiază</button><button class="btn small" onclick="dl(\'Raport_L4_\'+evTodayIsoSafe()+\'.txt\',window._repL4)">⬇ .txt</button><a class="btn small" href="'+mail("Raport săptămânal intern — "+new Date().toLocaleDateString("ro-RO"),l4)+'">✉ e-mail</a>'+exp)+'<div class="copybox" id="repL4">'+esc(l4)+'</div></div>';
  const hasMail=!!(rcl&&(rcl.email||(rcl.contact||{}).email));
  h+='<div class="card">'+cardHead('Raport per client','L5',CL.length?'<button class="btn small" onclick="copyTxt(window._repL5,\'Raport client copiat\')">📋 copiază</button><button class="btn small" onclick="dl(\'Raport_client_\'+evTodayIsoSafe()+\'.txt\',window._repL5)">⬇ .txt</button><a class="btn small" href="'+mail("Stadiu proiecte — "+(rcl?rcl.denumire:""),l5,hasMail?(rcl.email||(rcl.contact||{}).email||""):"")+'" title="'+(hasMail?'către adresa clientului din CRM':'fără adresă de e-mail în CRM — completezi destinatarul tu')+'">✉ e-mail</a>'+exp:'')
  ...
  h+='<div class="card section pj-tblcard">'+cardHead('KPI de operare',null,'<span class="evsrc">K2 · din datele reale</span>')+'<table class="tbl stack pj-static"><thead><tr><th>Indicator</th><th>Valoare</th><th>Țintă</th></tr></thead><tbody>'+kpi.map(r=>'<tr><td data-l="Indicator">'+r[0]+'</td><td data-l="Valoare">'+r[1]+'</td><td data-l="Țintă" class="pj-muted">'+r[2]+'</td></tr>').join("")+'</tbody></table></div>';
  return h+'</div>'; }
...
function vConformitate(){ const r=REF; const RF=S.riskFilter||""; const _rr=riskRegister(); const rows=_rr.rows.filter(x=>!RF||x.sev===RF); const nMaj=_rr.rows.filter(x=>x.sev==="MAJOR").length;
  ...
  const chips=[["","toate"],["MAJOR","MAJOR ("+nMaj+")"],["MINOR","MINOR ("+(_rr.rows.length-nMaj)+")"]].map(([k,l])=>'<button class="fchip'+(RF===k?" on":"")+'" onclick="S.riskFilter=\''+k+'\';render(true)">'+l+'</button>').join("");
  ...
  const ref=(t,sum,inner)=>'<details class="acc2"><summary><b>'+t+'</b><span class="pj-sum">'+sum+'</span></summary><div class="inner">'+inner+'</div></details>';
  ...
  h+=ref('Praguri achiziții 2026 ⏳','achiziție directă: '+nf.format(pp.achizitie_directa_produse_servicii_lei||0)+' lei produse/servicii · '+nf.format(pp.achizitie_directa_lucrari_lei||0)+' lei lucrări · privați: Ordin 1284/2016','<ul class="list">'+
   ...
   '<li style="color:var(--critical)">Sancțiuni: '+esc(pv.sanctiuni||"")+'</li></ul>');
  ...
  h+=ref('Durabilitate, arhivare & publicitate',esc(du.regula||"")+(ar.regula?' · arhivare: '+esc(ar.regula):''),'<ul class="list"><li><b>Durabilitate:</b> '+esc(du.regula||"")+'<br><small style="color:var(--critical)">'+esc(du.riscuri||"")+'</small></li><li><b>Arhivare:</b> '+esc(ar.regula||"")+'</li><li><b>Publicitate:</b> '+(pb.obligatorii||[]).map(esc).join(" · ")+' — sancțiune: '+esc(pb.sanctiune||"")+'</li></ul>');
  ...
  const pn=(r.pnrr_inchidere||{})["⏳"]||""; if(pn) h+=ref('PNRR — închidere ⏳',esc(pn),'<div style="font-size:13px;line-height:1.55">'+esc(pn)+'</div>');

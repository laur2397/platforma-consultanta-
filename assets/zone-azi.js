...
function vBuletin(){ const b=(META.buletin)||{}; const K=buletinKPI(); const _ra=radarAge(); const exDate=String((DB.apeluri||{}).extras_la||"").slice(0,10);
  ...
  let h='<div class="viewtitle"><h1>'+esc(h1t)+'</h1><span class="sub">'+sub+'</span><div class="viewactions az-va"><button class="btn small primary" onclick="copyBuletin()">📋 Copiază buletinul</button>'+moreMenu([["⬇ Export termene .ics (alertă T-7)","exportICS()"],["📅 Deschide Calendarul","S.view='calendar';render()"],["📡 Deschide Radarul","S.view='radar';render()"]])+'</div></div>';
  ...
  const termene='<div class="card">'+cardHead("Termenele următoarelor 14 zile",all14.length,'<button class="btn small ghost" onclick="S.view=\'calendar\';render()">Calendar →</button>')+miniCal(14)+'</div>';
  const urgN=(b.urgente||[]).length;
  const urg='<div class="card">'+cardHead("Urgențe azi",urgN)+(urgN?'<ul class="list">'+(b.urgente||[]).map(u=>li("❗",u)).join("")+'</ul>':'<div class="empty">Nicio urgență semnalată la ultima scanare.</div>')+'</div>';
  // SECUNDAR: oportunități + top potriviri
  const oppN=(b.oportunitati_cheie||[]).length;
  const opp='<div class="card">'+cardHead("Oportunități-cheie",oppN)+(oppN?'<ul class="list">'+(b.oportunitati_cheie||[]).map(u=>li("▸",u)).join("")+'</ul>':'<div class="empty">Nicio oportunitate semnalată la ultima scanare.</div>')+'</div>';
  const bp=bestPairs(6);
  const top='<div class="card">'+cardHead("Top potriviri client × apel",bp.length,'<button class="btn small ghost" onclick="S.view=\'matching\';render()">Matching →</button>')+(bp.length?'<table class="tbl stack"><thead><tr><th>Client</th><th>Apel</th><th>Termen</th><th>Verdict · scor</th></tr></thead><tbody>'+bp.map(m=>'<tr tabindex="0" onkeydown="if(event.key===\'Enter\')this.click()" onclick="openMemo(\''+m.client.id+'\',\''+esc(m.apel.id_apel)+'\')"><td data-l="Client"><b>'+esc(m.client.denumire)+'</b>'+(m.client.demo?' <span class="tag-demo">DEMO</span>':'')+'</td><td data-l="Apel">'+esc(m.apel.titlu)+'<br><small class="az-sub">'+esc(m.apel.program)+'</small></td><td data-l="Termen">'+cdBadge(m.apel.data_inchidere,{cont:/continuu/.test(m.apel.tip_depunere||"")})+'</td><td data-l="Verdict">'+vChip(m.verdict,m.scor)+'</td></tr>').join("")+'</tbody></table>':emptyState('🎯','Nicio potrivire încă','Motorul compară clienții din CRM cu apelurile din radar. Adaugă clienți ca să vezi perechile.','<button class="btn small" onclick="S.view=\'clienti\';render()">→ Clienți</button>'))+'</div>';
  // GRAFICE

...
function radarSave(){ try{ const f=S.radar; localStorage.setItem("eufcc_radar",JSON.stringify({q:f.q,stari:[...f.stari],benef:[...f.benef],regiune:f.regiune,program:f.program,sort:f.sort,dir:f.dir||1,verificat:f.verificat,maxDays:f.maxDays==null?null:f.maxDays,mode:f.mode,_touched:f._touched})); }catch(e){} }
function radarLoad(){ try{ const o=JSON.parse(localStorage.getItem("eufcc_radar")||"null"); if(!o) return; Object.assign(S.radar,{q:o.q||"",stari:new Set(o.stari||[]),benef:new Set(o.benef||[]),regiune:o.regiune||"",program:o.program||"",sort:o.sort||"termen",dir:o.dir||1,verificat:!!o.verificat,maxDays:o.maxDays==null?null:o.maxDays}); if(o._touched){ S.radar._touched=1; S.radar.mode=o.mode||"tabel"; } }catch(e){} }
...
function vRadar(){ const f=S.radar; const list=radarFiltered();
  ...
  let h='<div class="viewtitle"><h1>📡 Radar apeluri</h1><span class="sub"><span id="radarCount">'+list.length+' / '+A.length+'</span> apeluri · extras la '+fmtD((DB.apeluri||{}).extras_la?String(DB.apeluri.extras_la).slice(0,10):null)+'</span><div class="viewactions"><button class="btn small" onclick="radarCsv()" title="lista filtrată">⬇ CSV</button><button class="btn small '+(f.mode==="tabel"?"primary":"")+'" onclick="S.radar._touched=1;S.radar.mode=\'tabel\';render(true)">tabel</button><button class="btn small '+(f.mode==="carduri"?"primary":"")+'" onclick="S.radar._touched=1;S.radar.mode=\'carduri\';render(true)">carduri</button></div></div>';
  h+='<div class="presets"><span class="evsrc" style="align-self:center">Scenarii:</span>'+RADAR_PRESETS.map((p,i)=>'<button class="fchip preset'+(p[2](f)?" on":"")+'" onclick="radarPreset('+i+')">'+p[0]+(i?' <small>'+nAct(p[1])+'</small>':'')+'</button>').join("")+'</div>';
  h+='<div class="filters"><input type="search" placeholder="caută în radar (titlu, program, domeniu, sursă, note)…" value="'+esc(f.q)+'" oninput="S.radar.q=this.value;refreshRadar()" style="flex:1;min-width:200px">';
  h+='<select onchange="S.radar.regiune=this.value;render(true)"><option value="">— regiune —</option>'+REGIUNI.map(r=>'<option '+(f.regiune===r?"selected":"")+'>'+r+'</option>').join("")+'</select>';
  h+='<select onchange="S.radar.program=this.value;render(true)"><option value="">— program —</option>'+programs.map(p=>'<option '+(f.program===p?"selected":"")+'>'+esc(p)+'</option>').join("")+'</select>';
  h+='<span class="evsrc">Stare:</span>'+["activ","planificat","consultare"].map(st=>'<button class="fchip '+(f.stari.has(st)?"on":"")+'" onclick="tgSet(S.radar.stari,\''+st+'\');render(true)">'+RADAR_STL[st]+'</button>').join("");
  h+='<span class="evsrc">Beneficiar:</span>'+["UAT","IMM","microintreprindere","ONG","fermier","intreprindere_mare"].map(b=>'<button class="fchip '+(f.benef.has(b)?"on":"")+'" onclick="tgSet(S.radar.benef,\''+b+'\');render(true)">'+(BEN[b]||b)+'</button>').join("");
  h+='<span class="evsrc">Se închid în:</span>'+[[7,"≤7 zile"],[30,"≤30"],[90,"≤90"],[null,"oricând"]].map(([d,l])=>'<button class="fchip '+((f.maxDays==null?null:f.maxDays)===d?"on":"")+'" onclick="S.radar.maxDays='+(d===null?'null':d)+';render(true)">'+l+'</button>').join("");
  h+='<select onchange="S.radar.sort=this.value;render(true)">'+[["termen","sortare: termen"],["scor","sortare: scor portofoliu"],["program","sortare: program"],["grant","sortare: grant/buget"],["stare","sortare: stare"],["incredere","sortare: încredere"]].map(([k,l])=>'<option value="'+k+'" '+(f.sort===k?"selected":"")+'>'+l+'</option>').join("")+'</select><button class="btn small ghost" title="inversează ordinea" onclick="S.radar.dir=-(S.radar.dir||1);render(true)">'+((f.dir||1)<0?"▾ desc":"▴ asc")+'</button></div>';
...
function radarListHtml(list){ if(!list.length) return emptyState('📡','Niciun apel pe filtrele curente','Relaxează filtrele sau revino la toate apelurile.','<button class="btn primary" onclick="radarPreset(0)">Toate apelurile</button>');
  ...
  if(S.radar.mode==="carduri"){ return '<div class="grid3">'+list.map(a=>{const top=topForApel(a.id_apel,1)[0];
    return '<div class="card click" tabindex="0" onkeydown="if(event.key===\'Enter\')this.click()" onclick="openApel(\''+esc(a.id_apel)+'\')"><div style="display:flex;justify-content:space-between;gap:6px;align-items:flex-start"><b style="font-size:13.5px">'+esc(a.titlu)+'</b>'+stB(a.stare)+'</div><div style="color:var(--muted);font-size:12px;margin:3px 0 7px">'+esc(a.program)+' · '+radarRegs(a)+(a.corrigendum?' <span class="cd cd-warn">corr.</span>':'')+'</div><div>'+benChips(a.tip_beneficiar,3)+'</div><div style="margin-top:8px;display:flex;justify-content:space-between;align-items:center"><span style="font-size:12px;color:var(--ink2)">'+grantStr(a)+'</span>'+cdBadge(a.data_inchidere,{cont:/continuu/.test(a.tip_depunere||"")})+'</div>'+(top?'<div style="margin-top:7px;font-size:12px">🎯 '+esc(top.client.denumire)+' '+vChip(top.verdict,top.scor)+'</div>':"")+(conf(a)?'<div style="margin-top:5px">'+conf(a)+'</div>':"")+'</div>';}).join("")+'</div>'; }
  return '<div class="card" style="padding:4px 10px"><table class="tbl"><thead><tr>'+sh("titlu","Apel")+'<th>Beneficiari</th><th>Regiuni</th>'+sh("grant","Grant / buget","num")+sh("termen","Închidere")+sh("stare","Stare")+sh("scor","Top match")+'</tr></thead><tbody>'+
  list.map(a=>{const top=topForApel(a.id_apel,1)[0];
    return '<tr tabindex="0" onkeydown="if(event.key===\'Enter\')this.click()" onclick="openApel(\''+esc(a.id_apel)+'\')"><td style="max-width:330px"><b>'+esc(a.titlu)+'</b>'+(a.corrigendum?' <span class="cd cd-warn">corr.</span>':"")+conf(a)+(a.url_sursa?' <a href="'+esc(a.url_sursa)+'" target="_blank" rel="noopener" onclick="event.stopPropagation()" title="sursa oficială" style="font-size:11px">↗</a>':'')+'<br><small style="color:var(--muted)">'+esc(a.program)+'</small></td><td>'+benChips(a.tip_beneficiar,2)+'</td><td style="font-size:12px">'+radarRegs(a)+'</td><td class="num" style="font-size:12px">'+grantStr(a)+'</td><td>'+cdBadge(a.data_inchidere,{cont:/continuu/.test(a.tip_depunere||"")})+'</td><td>'+stB(a.stare)+'</td><td style="font-size:12px;max-width:160px"><span title="'+(top?esc(top.client.denumire):'')+'" style="display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+(top?esc(top.client.denumire)+" · <b>"+top.scor+"</b>":"—")+'</span></td></tr>';}).join("")+'</tbody></table></div>'; }
...
function vMatching(){ if(!CL.length) return '<div class="viewtitle"><h1>🎯 Matching & prioritizare</h1></div>'+emptyState('🎯','Niciun client în CRM','Motorul de matching compară clienții tăi cu toate apelurile din radar. Adaugă sau importă clienți ca să vezi potrivirile.','<button class="btn primary" onclick="S.view=\'clienti\';render()">→ Clienți</button>'+(window.CRM.hideDemo?'<button class="btn ghost" onclick="crmToggleDemo()">👁 arată clienții demo</button>':''));
  ...
  let h='<div class="viewtitle"><h1>🎯 Matching & prioritizare</h1><span class="sub">motor în 2 trepte: criterii HARD (eliminatorii) + scor SOFT 0-100 · estimativ, nevalidat uman</span><div class="viewactions"><button class="btn small" onclick="matchCsv()">⬇ CSV matrice</button></div></div>';
  h+='<div class="grid2">';
  h+='<div class="card"><h2>Per client → top apeluri</h2><div style="display:flex;gap:6px;align-items:center"><select style="flex:1" onchange="S.matchClient=this.value;render(true)">'+CL.map(x=>'<option value="'+x.id+'" '+(x.id===cid?"selected":"")+'>'+esc(x.denumire)+(x.demo?' · DEMO':'')+'</option>').join("")+'</select><button class="btn small" title="fișa clientului" onclick="openClient(\''+cid+'\')">👥</button></div>';
  h+='<div class="evsub" style="margin:8px 0 4px"><button class="fchip'+(M.onlyActive?" on":"")+'" onclick="S.match.onlyActive=!S.match.onlyActive;render(true)">doar deschise acum</button><button class="fchip'+(M.sort==="scor"?" on":"")+'" onclick="S.match.sort=\'scor\';render(true)">după scor</button><button class="fchip'+(M.sort==="termen"?" on":"")+'" onclick="S.match.sort=\'termen\';render(true)">după termen</button></div><div style="margin-top:6px" id="mcList">'+matchClientHtml(cid)+'</div></div>';
  h+='<div class="card"><h2>Per apel → top clienți</h2><select style="width:100%" onchange="S.matchApel=this.value;render(true)"><option value="">— alege apelul —</option>'+Object.keys(groups).sort((x,y)=>x.localeCompare(y,"ro")).map(g=>'<optgroup label="'+esc(g)+'">'+groups[g].map(a=>'<option value="'+esc(a.id_apel)+'" '+(a.id_apel===S.matchApel?"selected":"")+'>'+esc(a.titlu.slice(0,80))+(a.data_inchidere?' · '+fmtDs(a.data_inchidere):'')+'</option>').join("")+'</optgroup>').join("")+'</select><div style="margin-top:10px">'+(S.matchApel?matchApelHtml(S.matchApel):'<div class="evsrc" style="margin-bottom:6px">Până alegi un apel — cele mai bune perechi din tot portofoliul:</div>'+matchBestHtml())+'</div></div>';
  h+='</div>';
  ...
  return h; }
function matchBar(s){ return '<span class="minibar" style="display:inline-block;width:44px;height:6px;vertical-align:middle;margin-left:6px"><span style="width:'+Math.max(0,Math.min(100,s||0))+'%;background:'+(s>=70?'var(--good)':s>=40?'var(--warn)':'var(--critical)')+'"></span></span>'; }
function matchClientHtml(cid){ const c=clientById(cid); if(!c) return ""; const M=S.match||{};
  ...
  let h= ok.length? '<table class="tbl"><thead><tr><th>Apel</th><th>Termen</th><th>Verdict</th><th>Verif.</th></tr></thead><tbody>'+ok.map(m=>{ const t=m.apel.titlu||""; return '<tr onclick="S.matchApel=\''+esc(m.apel.id_apel)+'\';openMemo(\''+cid+'\',\''+esc(m.apel.id_apel)+'\')"><td><b title="'+esc(t)+'">'+esc(t.length>90?t.slice(0,90)+"…":t)+'</b><br><small style="color:var(--muted)">'+esc(m.apel.program)+'</small></td><td>'+cdBadge(m.apel.data_inchidere,{cont:/continuu/.test(m.apel.tip_depunere||"")})+'</td><td style="white-space:nowrap">'+vChip(m.verdict,m.scor)+matchBar(m.scor)+'</td><td><span class="chip" title="'+esc((m.unknowns||[]).join("\n"))+'">'+(m.unknowns||[]).length+' verif.</span></td></tr>'; }).join("")+'</tbody></table>' : '<div class="empty">Niciun apel eligibil pe datele curente'+(M.onlyActive?' (doar deschise)':'')+'.</div>';
  if(bad.length) h+='<details class="acc2" style="margin-top:10px"><summary>NO-GO relevante (gap analysis) <span class="cd cd-crit">'+bad.length+'</span></summary><div class="inner">'+bad.map(m=>'<div style="margin-bottom:8px"><b>'+esc(m.apel.titlu)+'</b><br><small style="color:var(--critical)">'+m.fails.map(esc).join(" · ")+'</small></div>').join("")+'</div></details>';
  return h; }
function matchApelHtml(aid){ const tops=matchAll().filter(m=>m.apel.id_apel===aid).sort((x,y)=>y.scor-x.scor).slice(0,10);
  ...
  return '<table class="tbl"><thead><tr><th>Client</th><th>Verdict</th><th>Motiv principal</th></tr></thead><tbody>'+tops.map(m=>'<tr onclick="openMemo(\''+m.client.id+'\',\''+esc(aid)+'\')"><td><b>'+esc(m.client.denumire)+'</b>'+(m.client.demo?' <span class="tag-demo">DEMO</span>':'')+'<br><small style="color:var(--muted)">'+esc(m.client.judet||"")+'</small></td><td style="white-space:nowrap">'+vChip(m.verdict,m.scor)+matchBar(m.scor)+'</td><td style="font-size:12px;color:var(--ink2)">'+esc(m.fails[0]||(m.unknowns||[])[0]||m.note[0]||"—")+'</td></tr>').join("")+'</tbody></table>'; }
function matchBestHtml(){ const bp=bestPairs(8); if(!bp.length) return '<div class="empty">Nicio potrivire în portofoliu.</div>'; return '<table class="tbl"><thead><tr><th>Client × apel</th><th>Termen</th><th>Verdict</th></tr></thead><tbody>'+bp.map(m=>'<tr onclick="S.matchApel=\''+esc(m.apel.id_apel)+'\';openMemo(\''+m.client.id+'\',\''+esc(m.apel.id_apel)+'\')"><td><b>'+esc(m.client.denumire)+'</b><br><small style="color:var(--muted)">'+esc(m.apel.titlu.slice(0,70))+'</small></td><td>'+cdBadge(m.apel.data_inchidere,{cont:/continuu/.test(m.apel.tip_depunere||"")})+'</td><td style="white-space:nowrap">'+vChip(m.verdict,m.scor)+matchBar(m.scor)+'</td></tr>').join("")+'</tbody></table>'; }
...
/* ---------- Pipeline ---------- */
function vPipeline(){ const cols=Object.keys(FAZE); const _pc=crmProjCounts(); const F=S.pipe||(S.pipe={q:"",consultant:"",client:"",red:false});
  let h='<div class="viewtitle"><h1>📋 Pipeline proiecte</h1><span class="sub">'+_pc.reali+' reale'+(_pc.demo?' · '+_pc.demo+' demo':"")+' · P0→P9</span><div class="viewactions"><button class="btn small primary" onclick="crmProjForm()">+ Proiect nou</button><button class="btn small" onclick="crmImportOpen()">⬆ Import</button><button class="btn small" onclick="pipeCsv()">⬇ CSV</button>'+(_pc.demo||window.CRM.hideDemo?'<button class="btn small ghost" onclick="crmToggleDemo()">'+(window.CRM.hideDemo?"👁 arată demo":"🙈 ascunde demo")+'</button>':"")+'</div></div>';
  if(!PR.length) return h+emptyState('📋','Niciun proiect în pipeline','Adaugă primul proiect sau alege un apel din Radar și apasă „➕ Proiect în pipeline”.','<button class="btn primary" onclick="crmProjForm()">+ Proiect nou</button><button class="btn" onclick="S.view=\'radar\';render()">📡 Radar</button>'+(window.CRM.hideDemo?'<button class="btn ghost" onclick="crmToggleDemo()">👁 arată demo</button>':''));
  ...
  h+='<div class="filters"><input type="search" placeholder="caută proiect, client…" value="'+esc(F.q)+'" oninput="S.pipe.q=this.value;render(true)" style="flex:1;min-width:180px"><select onchange="S.pipe.consultant=this.value;render(true)"><option value="">— consultant —</option>'+cons.map(x=>'<option '+(F.consultant===x?"selected":"")+'>'+esc(x)+'</option>').join("")+'</select><select onchange="S.pipe.client=this.value;render(true)"><option value="">— client —</option>'+clis.map(c=>'<option value="'+esc(c.id)+'" '+(F.client===c.id?"selected":"")+'>'+esc(c.denumire)+'</option>').join("")+'</select>'+(F.red?'<button class="fchip on" onclick="S.pipe.red=false;render(true)">🔴 doar în roșu ✕</button>':'')+'<details class="acc2" style="margin-left:auto;border:0;background:transparent"><summary style="padding:4px 8px;font-weight:500;color:var(--muted);font-size:12px">reguli sănătate</summary><div class="inner" style="font-size:12px;color:var(--ink2)">🔴 acțiune restantă sau apelul se închide ≤7 zile în faza de pregătire sau clarificări (P5) · 🟡 acțiune în ≤5 zile sau apel ≤21 zile · trage cardul în altă coloană ca să schimbi faza.</div></details></div>';
  ...
  h+='<div class="grid2 section"><div class="card"><h2>Valoare grant pe fază</h2>'+chartHBars(fazeVal,{fmt:v=>money(v,"lei")})+'</div><div class="card"><h2>Comisioane prognozate pe fază</h2>'+chartHBars(comVal.map(x=>Object.assign({},x,{c:"var(--s3)"})),{fmt:v=>money(v,"lei")})+'</div></div>';
  return h; }
...
function vCalendar(){ const K=S.calKinds||(S.calKinds=new Set(["inchidere","deschidere","proiect"])); const hor=S.calHor||60;
  let h='<div class="viewtitle"><h1>📅 Calendar & termene</h1><span class="sub">închideri și deschideri de apeluri + obligațiile proiectelor</span><div class="viewactions"><button class="btn small '+(S.calMode==="lista"?"primary":"")+'" onclick="S.calMode=\'lista\';render(true)">listă</button><button class="btn small '+(S.calMode==="luna"?"primary":"")+'" onclick="S.calMode=\'luna\';render(true)">lună</button><button class="btn small" onclick="exportICS()" title="Google / Outlook / Apple — cu alertă T-7 pentru apeluri">⬇ Export .ics</button></div></div>';
  h+='<div class="filters"><span class="evsrc">Arată:</span>'+[["inchidere","Închideri apeluri","k-inchidere"],["deschidere","Deschideri apeluri","k-deschidere"],["proiect","Obligații proiecte","k-proiect"]].map(([k,l,c])=>'<button class="fchip '+(K.has(k)?"on":"")+'" onclick="tgSet(S.calKinds,\''+k+'\');render(true)"><span class="kdot '+c+'"></span>'+l+'</button>').join("")+'<button class="fchip '+(S.calMatch?"on":"")+'" title="doar apelurile cu cel puțin un client potrivit" onclick="S.calMatch=!S.calMatch;render(true)">🎯 cu potrivire în portofoliu</button>'+(S.calMode==="lista"?'<span class="evsrc" style="margin-left:auto">Orizont:</span>'+[30,60,90,180].map(d=>'<button class="fchip '+(hor===d?"on":"")+'" onclick="S.calHor='+d+';render(true)">'+d+' zile</button>').join(""):'')+'</div>';
  h+= S.calMode==="lista"? calList(hor) : calMonth();
  h+='<div class="callout" style="margin-top:12px">Praguri: ≤7 zile roșu · 8–30 portocaliu · &gt;30 verde. Exportul .ics pune o alertă la T-7 pe închiderile de apeluri. Termenele de <b>clarificări</b> (3-5 zile lucrătoare) au prioritate absolută — apar aici doar dacă sunt înregistrate pe proiect.</div>';
  return h; }
...
function calRow(it){ const go=it.kind==="apel"?"openApel('"+esc(it.id)+"')":"openProiect('"+esc(it.id)+"')"; return '<div class="calit '+calKindCls(it)+'" style="cursor:pointer" onclick="'+go+'"><span class="dt">'+fmtDs(it.data)+'</span><div style="flex:1;min-width:0"><span class="tp '+calKindCls(it)+'">'+esc(it.tip)+'</span><br><b>'+esc(it.titlu)+'</b>'+(it.sub?' <small style="color:var(--muted)">— '+esc(it.sub)+'</small>':"")+'</div>'+cdBadge(it.data,{task:it.kind==="proiect"})+'<a class="btn small ghost" href="'+calGcal(it)+'" target="_blank" rel="noopener" title="adaugă în Google Calendar" onclick="event.stopPropagation()">📅+</a></div>'; }
function calList(hor){ const items=calFiltered(hor||60); const late=(S.calKinds.has("proiect")?TS.filter(t=>{const d=days(t.data);return d!=null&&d<0&&d>=-90&&PR.some(p=>p.id===t.proiect_id);}).map(t=>{const p=PR.find(x=>x.id===t.proiect_id);return {data:t.data,tip:(t.tip||"").replace(/_/g," "),titlu:t.descriere,sub:p?(clientById(p.client_id)||{}).denumire:"",kind:"proiect",id:t.proiect_id};}):[]).concat(PR.filter(p=>p.next_action&&p.next_action.termen&&days(p.next_action.termen)<0).map(p=>({data:p.next_action.termen,tip:"next action",titlu:p.next_action.descriere,sub:(clientById(p.client_id)||{}).denumire,kind:"proiect",id:p.id})));
  let h=''; if(late.length&&S.calKinds.has("proiect")) h+='<div class="calweek"><h3 style="color:var(--critical)">Restante ('+late.length+')</h3>'+late.sort((a,b)=>(a.data||"").localeCompare(b.data||"")).map(calRow).join("")+'</div>';
  if(!items.length) return h+'<div class="empty">Nimic în '+(hor||60)+' de zile pe filtrele curente.</div>';
  ...
  return h+out.map(k=>{ const m=pd(k); const end=new Date(m); end.setDate(m.getDate()+6); const now=iso(start)===k; const list=wk[k]||[];
    return '<div class="calweek'+(now?' now':'')+'"><h3>'+(now?'Săptămâna aceasta · ':'Săptămâna ')+m.toLocaleDateString("ro-RO",{day:"numeric",month:"short"})+' – '+end.toLocaleDateString("ro-RO",{day:"numeric",month:"short"})+(list.length?' <span class="evsrc">('+list.length+')</span>':'')+'</h3>'+(list.length?list.map(calRow).join(""):'<div class="evsrc" style="padding:4px 12px 8px">nimic programat</div>')+'</div>';}).join(""); }
function calMonth(){ const m=S.calMonth; const y=m.getFullYear(), mo=m.getMonth();
  ...
  for(let d=1;d<=dim;d++){ const ds=y+"-"+String(mo+1).padStart(2,"0")+"-"+String(d).padStart(2,"0"); const its=byDate[ds]||[]; const isT=pd(ds).getTime()===TODAY.getTime();
    h+='<div class="mcell'+(isT?" today":"")+'"'+(its.length?' style="cursor:pointer" onclick="calDay(\''+ds+'\')"':'')+'><span class="dn">'+d+'</span>'+its.slice(0,3).map(it=>'<span class="ev '+calKindCls(it)+(it.crit?" crit":it.warn?" warn":"")+'" title="'+esc(it.tip+": "+it.titlu)+'" onclick="event.stopPropagation();'+(it.kind==="apel"?"openApel('"+esc(it.id)+"')":"openProiect('"+esc(it.id)+"')")+'">'+esc(it.titlu.slice(0,22))+'</span>').join("")+(its.length>3?'<span class="ev more">+'+(its.length-3)+' — vezi ziua</span>':"")+'</div>'; }
...
function vBiblioteca(){ const ghid=A.filter(a=>a.url_ghid); const ord={activ:0,planificat:1,consultare:2,in_evaluare:3,inchis:4}; ghid.sort((x,y)=>(ord[x.stare]??9)-(ord[y.stare]??9)||(x.data_inchidere||"9").localeCompare(y.data_inchidere||"9"));
  ...
  let h='<div class="viewtitle"><h1>📚 Bibliotecă ghiduri & șabloane</h1><span class="sub">ghiduri per apel · corrigende · șabloane L1–L7</span><div class="viewactions"><button class="btn small" onclick="document.getElementById(\'bibCorr\').scrollIntoView({behavior:\'smooth\'})">Corrigende ('+corr.length+')</button><button class="btn small" onclick="document.getElementById(\'bibGhid\').scrollIntoView({behavior:\'smooth\'})">Ghiduri ('+ghid.length+')</button><button class="btn small" onclick="document.getElementById(\'bibSab\').scrollIntoView({behavior:\'smooth\'})">Șabloane</button></div></div>';
  h+='<div class="section" id="bibCorr"><h2>Corrigende detectate</h2>';
  h+= corr.length? corr.map(a=>{ const aff=PR.filter(p=>p.apel_id===a.id_apel).length; const tops=topForApel(a.id_apel,3).map(m=>m.client.denumire); return '<div class="calit" style="cursor:pointer" onclick="openApel(\''+esc(a.id_apel)+'\')"><span class="dt" title="corrigendum semnalat în radar"><span class="cd cd-warn">⚠ corr.</span></span><div style="flex:1"><span class="tp">'+esc(a.program||"")+'</span><br><b>'+esc(a.titlu)+'</b><br><small class="clamp" style="color:var(--ink2)">'+esc(a.note||"")+'</small>'+(aff||tops.length?'<div style="margin-top:4px;font-size:12px">'+(aff?'<span class="chip hl">'+aff+' proiect(e) în pipeline</span>':'')+tops.map(t=>'<span class="chip">'+esc(t)+'</span>').join('')+'</div>':'')+'</div>'+stB(a.stare)+'</div>'; }).join("") : '<div class="empty">Niciun corrigendum activ.</div>';
  h+='</div>';
  h+='<div class="section" id="bibGhid"><h2>Ghiduri disponibile din radar ('+ghid.length+')</h2><div class="card" style="padding:4px 10px"><table class="tbl"><thead><tr><th>Apel</th><th>Ghid</th><th>Termen</th><th>Stare</th><th></th></tr></thead><tbody>'+ghid.map(a=>{ let host=""; try{ host=new URL(a.url_ghid).hostname.replace(/^www\./,""); }catch(e){} return '<tr onclick="openApel(\''+esc(a.id_apel)+'\')"><td><b>'+esc(a.titlu)+'</b><br><small style="color:var(--muted)">'+esc(a.program)+'</small></td><td><a href="'+esc(a.url_ghid)+'" target="_blank" onclick="event.stopPropagation()">deschide ghidul ↗</a><br><small style="color:var(--muted)">'+esc(host)+'</small></td><td>'+cdBadge(a.data_inchidere,{cont:/continuu/.test(a.tip_depunere||"")})+'</td><td>'+stB(a.stare)+'</td><td><button class="btn small ghost" onclick="event.stopPropagation();genL1(\''+esc(a.id_apel)+'\')" title="generează fișa L1 din datele apelului">📋 L1</button></td></tr>'; }).join("")+'</tbody></table></div>';
  h+='<details class="acc2" style="margin-top:8px"><summary>Apeluri fără ghid indexat ('+fara.length+') <span class="evsrc">— link-ul ghidului nu a fost extras; verifică la sursă</span></summary><div class="inner"><ul class="list">'+fara.slice(0,60).map(a=>'<li style="cursor:pointer" onclick="openApel(\''+esc(a.id_apel)+'\')"><span style="flex:1"><b>'+esc(a.titlu)+'</b> <small style="color:var(--muted)">'+esc(a.program||"")+'</small></span>'+(a.url_sursa?'<a href="'+esc(a.url_sursa)+'" target="_blank" onclick="event.stopPropagation()" style="font-size:12px">sursă ↗</a>':'')+'</li>').join("")+(fara.length>60?'<li style="color:var(--muted)">+ încă '+(fara.length-60)+' în Radar</li>':'')+'</ul></div></details>';
  h+='<div class="callout" style="margin-top:8px">Pentru apelurile pe care lucrează firma: descarcă PDF-ul ghidului, atașează-l în conversația Claude și cere „Q&A pe ghid" — răspunsurile vor cita pagina/secțiunea, niciodată din memorie.</div></div>';
  h+='<div class="section" id="bibSab"><h2>Șabloane L1–L7 (formate obligatorii)</h2><div class="card" style="margin-bottom:10px"><div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap"><b style="font-size:13px">Generează fișa L1 pentru un apel:</b><select id="bibL1" style="flex:1;min-width:220px">'+A.filter(a=>a.stare!=="inchis").map(a=>'<option value="'+esc(a.id_apel)+'">'+esc((a.titlu||"").slice(0,70))+' · '+esc(a.program||"")+'</option>').join("")+'</select><button class="btn small primary" onclick="genL1(document.getElementById(\'bibL1\').value)">📋 Generează & copiază</button></div><div class="evsrc" style="margin-top:6px">Completează automat câmpurile cunoscute din radar; ce lipsește rămâne marcat „… [DE VERIFICAT]” — nimic inventat.</div></div>'
    +Object.keys(SABLOANE).map(k=>{ const kk=k.replace(/'/g,"\\'"); const first=SABLOANE[k].split("\n")[1]||""; return '<details class="acc2"><summary style="display:flex;gap:8px;align-items:center"><span style="flex:1">'+esc(k)+' <small style="color:var(--muted);font-weight:400">'+esc(first.slice(0,60))+'</small></span><button class="btn small ghost" onclick="event.preventDefault();event.stopPropagation();copyTxt(SABLOANE[\''+kk+'\'],\'Șablon copiat\')">📋</button></summary><div class="inner"><div class="copybox">'+esc(SABLOANE[k])+'</div><div style="display:flex;gap:6px;margin-top:8px"><button class="btn small" onclick="copyTxt(SABLOANE[\''+kk+'\'],\'Șablon copiat\')">📋 copiază</button><button class="btn small ghost" onclick="dl(\''+esc(k.replace(/[^\w]+/g,"_"))+'.txt\',SABLOANE[\''+kk+'\'])">⬇ .txt</button></div></div></details>'; }).join("")+'</div>';
  return h; }
...
function vRapoarte(){ if(!clientById(S.repClient)) S.repClient=CL[0]?CL[0].id:null; const rc=S.repClient;
  ...
  let h='<div class="viewtitle"><h1>📊 Centru de rapoarte</h1><span class="sub">generate din datele la zi ('+new Date().toLocaleDateString("ro-RO")+') — de validat uman înainte de trimitere</span></div>';
  h+='<div class="grid2"><div class="card"><h2>Raport săptămânal intern (L4)</h2><div class="rep-actions"><button class="btn small primary" onclick="copyTxt(window._repL4,\'Raport L4 copiat\')">📋 copiază</button><button class="btn small" onclick="dl(\'Raport_L4_\'+evTodayIsoSafe()+\'.txt\',window._repL4)">⬇ .txt</button><a class="btn small" href="'+mail("Raport săptămânal intern — "+new Date().toLocaleDateString("ro-RO"),l4)+'">✉ e-mail</a><button class="btn small ghost" onclick="this.closest(\'.card\').querySelector(\'.copybox\').classList.toggle(\'full\')">⤢ extinde</button></div><div class="copybox" id="repL4">'+esc(l4)+'</div></div>';
  h+='<div class="card"><h2>Raport per client (L5)</h2>'+(CL.length?'<select style="width:100%" onchange="S.repClient=this.value;render(true)">'+CL.map(c=>'<option value="'+c.id+'" '+(c.id===rc?"selected":"")+'>'+esc(c.denumire)+(c.demo?' · DEMO':'')+'</option>').join("")+'</select><div class="rep-actions" style="margin-top:8px"><button class="btn small primary" onclick="copyTxt(window._repL5,\'Raport client copiat\')">📋 copiază</button><button class="btn small" onclick="dl(\'Raport_client_\'+evTodayIsoSafe()+\'.txt\',window._repL5)">⬇ .txt</button><a class="btn small" href="'+mail("Stadiu proiecte — "+(rcl?rcl.denumire:""),l5,rcl?(rcl.email||(rcl.contact||{}).email||""):"")+'">✉ e-mail'+(rcl&&(rcl.email||(rcl.contact||{}).email)?' către client':'')+'</a><button class="btn small ghost" onclick="this.closest(\'.card\').querySelector(\'.copybox\').classList.toggle(\'full\')">⤢ extinde</button></div>'+(rcl&&rcl.demo?'<div class="callout warn" style="margin-top:8px">Client <b>demo</b> — textul e marcat [DEMO] și nu trebuie trimis nimănui.</div>':'')+'<div class="copybox" style="margin-top:8px">'+esc(l5)+'</div>':emptyState('👥','Niciun client în CRM','Rapoartele per client se generează din proiectele clienților tăi.','<button class="btn primary" onclick="S.view=\'clienti\';render()">→ Clienți</button>'))+'</div></div>';
  ...
  h+='<div class="section card"><h2>KPI de operare (K2) — din datele reale</h2><table class="tbl"><thead><tr><th>Indicator</th><th>Valoare</th><th>Țintă</th></tr></thead><tbody>'+
  [["Prospețimea radarului",(()=>{const a=radarAge();return a?'<span class="cd '+(a.cls==="good"?"cd-good":a.cls==="warn"?"cd-warn":"cd-crit")+'">scanat acum '+a.zile+' zile</span>':"—";})(),"≤ 2 zile"],
   ...
   ["Termene interne ratate (acțiuni restante)",ratate?'<span class="cd cd-crit">'+ratate+'</span>':(reale.length?'<span class="cd cd-good">0</span>':'<span style="color:var(--muted)">— (fără proiecte reale)</span>'),"0"],
   ["Corecții financiare suferite",'<span style="color:var(--muted)" title="nu se urmărește automat — se completează manual din deciziile AM">— de completat manual</span>',"0 lei"],
   ["Valoare pipeline real (grant)",reale.length?money(reale.reduce((s,p)=>s+(p.grant_lei||0),0),"lei"):'<span style="color:var(--muted)">— (doar demo)</span>',"↑"]].map(r=>'<tr><td>'+r[0]+'</td><td>'+r[1]+'</td><td style="color:var(--muted)">'+r[2]+'</td></tr>').join("")+'</tbody></table></div>';
  return h; }
function raportL4(){ const d=new Date().toLocaleDateString("ro-RO"); const cl7=A.filter(a=>{const x=days(a.data_inchidere);return x!=null&&x>=0&&x<=7;});
  ...
  Object.keys(FAZE).forEach(f=>{ const ps=PRx.filter(p=>p.faza===f); if(ps.length) t+="• "+f+" "+FAZE[f]+": "+ps.length+" proiect(e), grant "+money(ps.reduce((s,p)=>s+(p.grant_lei||0),0),"lei")+"\n"; });
...
function vConformitate(){ const r=REF; const RF=S.riskFilter||"";
  let h='<div class="viewtitle"><h1>🛡️ Conformitate & audit</h1><span class="sub">'+esc(r.nota||"referențial D5")+' · elementele ⏳ se reverifică periodic</span></div>';
  const _rr=riskRegister(); const rows=_rr.rows.filter(x=>!RF||x.sev===RF); const nMaj=_rr.rows.filter(x=>x.sev==="MAJOR").length;
  h+='<div class="section"><div class="evsech"><h2>Registru de riscuri (proiecte active) — '+_rr.rows.length+'</h2><div class="evsub" style="margin:0">'+[["","toate"],["MAJOR","MAJOR ("+nMaj+")"],["MINOR","MINOR ("+(_rr.rows.length-nMaj)+")"]].map(([k,l])=>'<button class="fchip'+(RF===k?" on":"")+'" onclick="S.riskFilter=\''+k+'\';render(true)">'+l+'</button>').join("")+'</div></div>';
  ...
  if(!rows.length) h+='<div class="card"><div class="empty">'+(_rr.rows.length?'Niciun risc pentru filtrul curent.':'Niciun risc detectat automat pe proiectele reale. 🎉 Continuă monitorizarea termenelor și a plafonului de minimis.')+'</div></div>';
  else h+='<div class="card riskcard" style="padding:4px 10px"><table class="tbl stack"><thead><tr><th>Proiect</th><th>Sev.</th><th>Risc</th><th>Expunere potențială</th><th>Măsură</th></tr></thead><tbody>'+rows.map(x=>{ const real=PR.some(p=>p.id===x.proiect); return '<tr'+(real?' onclick="openProiect(\''+esc(x.proiect)+'\')"':' style="cursor:default" title="risc demo — fără proiect real"')+'><td data-l="Proiect">'+esc(x.proiect)+(x.demo?' <span class="tag-demo">DEMO</span>':'')+'</td><td data-l="Sev."><span class="cd '+(x.sev==="MAJOR"?"cd-crit":"cd-warn")+'">'+esc(x.sev)+'</span></td><td data-l="Risc">'+esc(x.risc)+'</td><td data-l="Expunere"><b>'+esc(x.expunere)+'</b></td><td data-l="Măsură" style="font-size:12px">'+esc(x.masura)+'</td></tr>'; }).join("")+'</tbody></table></div>';
  h+='</div>';
  const priv=CL.filter(c=>c.tip!=="UAT"); if(!S.msClient||!priv.some(c=>c.id===S.msClient)) S.msClient=priv[0]?priv[0].id:"";
  h+='<div class="grid2 section"><div class="card" id="msCard"><h2>Simulator minimis (întreprindere unică)</h2>'+(priv.length?'<select id="msClient" style="width:100%" onchange="S.msClient=this.value;msRender()">'+priv.map(c=>'<option value="'+c.id+'"'+(c.id===S.msClient?" selected":"")+'>'+esc(c.denumire)+(c.demo?' · DEMO':'')+'</option>').join("")+'</select><div id="msBox" style="margin-top:10px"></div>':'<div class="empty">Adaugă un client privat în CRM pentru simulare.</div>')+'<div class="callout warn" style="margin-top:8px">Plafon 300.000 EUR / orice 3 ani (glisant) / <b>întreprindere unică</b> — tot grupul legat. Verificarea finală: RegAS + declarații. Interzis: minimis pentru vehicule de transport marfă.</div></div>';
  h+='<div class="card"><h2>Intensități GBER pe județ (harta 2022–2027)</h2><select id="gberJud" style="width:100%" onchange="S.gberJud=this.value;gberRender()"><option value="">— alege județul —</option>'+gberAllJud().map(j=>'<option'+(j===S.gberJud?" selected":"")+'>'+j+'</option>').join("")+'</select><div id="gberBox" style="margin-top:10px" class="empty">Alege județul pentru intensitatea maximă.</div><div style="font-size:11.5px;color:var(--muted);margin-top:8px">Bonus IMM: +20pp micro/mici, +10pp mijlocii (≤50 mil EUR) · +10pp în teritorii de tranziție justă · Efect stimulativ: depunere ÎNAINTE de începerea lucrărilor. Plafonul efectiv îl stabilește ghidul apelului.</div></div></div>';
  ...
  h+='<div class="grid2 section"><div class="card"><div class="evsech"><h2>Checklist pre-depunere (simulare CAE)</h2>'+ckp("cae",r.checklist_pre_depunere_cae||[])+'</div>'+ckList("cae",r.checklist_pre_depunere_cae||[])+'<div style="margin-top:6px"><button class="btn small ghost" onclick="S.checklists.cae={};ckTg(\'cae\',-1)">↺ resetează</button></div></div>';
...
/* Verificare proiect: vezi assets/evaluator.js (v2) */

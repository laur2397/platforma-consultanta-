...
function openClient(cid){ const c=clientById(cid); if(!c) return;
  ...
  if(!c.demo) h+='<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px"><button class="btn small primary" onclick="crmProjForm(null,\''+esc(c.id)+'\')">➕ Proiect nou</button><button class="btn small" onclick="S.view=\'verif\';render();evChooseClient(\''+esc(c.id)+'\')">🧪 Verifică un proiect</button><button class="btn small" onclick="finOpenFor(\''+esc(c.id)+'\')" title="fișa financiară a clientului (bilanț, P&P, IMM, buget)">🧮 Financiar</button><button class="btn small" onclick="crmNewForm(\''+esc(c.id)+'\')">✎ Editează</button><button class="btn small ghost" style="margin-left:auto;color:var(--critical)" onclick="crmDeleteClient(\''+esc(c.id)+'\')">🗑 Șterge</button></div>';
  const flags=[];
...
/* Ajutor: ghidul celor 5 zone + primii pași; ecran de bun venit la prima vizită */
function helpOpen(first){ const z=[["azi","Azi","Ce am de făcut azi: termene, urgențe, cifrele zilei."],["apeluri","Apeluri","Radarul finanțărilor, calendarul termenelor, potrivirea cu clienții, ghiduri și șabloane."],["clienti","Clienți","Portofoliul tău (CRM), prospectarea în registrul firmelor, piața pe județe."],["proiecte","Proiecte","Pipeline-ul dosarelor, verificarea pre-depunere, rapoarte, riscuri și conformitate."],["instrumente","Instrumente","Calcule financiare (machetă, buget, deviz), registre publice, setări și backup."]];
  let h=drawerHead(first?"Bine ai venit":"Cum folosesc platforma", first?"30 de secunde de orientare":"5 zone · 3 primi pași · scurtături")+'<div class="db">';
  if(first) h+='<div class="callout good">Totul rulează în browserul tău. Clienții, proiectele și fișierele ONRC rămân pe acest dispozitiv — nu pleacă nicăieri.</div>';
  h+='<div class="section"><h2>Cele 5 zone</h2><ul class="list">'+z.map(([id,l,d])=>'<li style="cursor:pointer" onclick="closeDrawer();zoneGo(\''+id+'\')"><span class="vi" style="width:30px;height:30px;border-radius:8px;background:var(--accent-soft);color:var(--accent-ink);display:grid;place-items:center;flex-shrink:0">'+ico(ZONES.find(x=>x[0]===id)[4])+'</span><span style="flex:1"><b>'+l+'</b><br><span style="color:var(--ink2);font-size:12.5px">'+d+'</span></span><span style="color:var(--accent)">→</span></li>').join("")+'</ul></div>';
  h+='<div class="section"><h2>Primii pași</h2><ol class="evplan"><li class="w"><div><b>Adaugă clienții tăi</b><div>Manual sau din Excel/CSV. Până atunci vezi date demo, marcate explicit.</div></div><button class="btn small primary" onclick="closeDrawer();S.view=\'clienti\';render();crmNewForm()">Client nou</button></li><li class="w"><div><b>Vezi ce apeluri li se potrivesc</b><div>Matching-ul compară automat fiecare client cu toate apelurile din radar.</div></div><button class="btn small" onclick="closeDrawer();S.view=\'matching\';render()">Matching</button></li><li class="w"><div><b>Verifică un dosar înainte de depunere</b><div>Reguli pre-încărcate din apel + legislație; verdict live cu sursă.</div></div><button class="btn small" onclick="closeDrawer();S.view=\'verif\';render()">Verificare</button></li></ol></div>';
  h+='<div class="section"><h2>Scurtături</h2><dl class="kv"><dt><span class="kbd" style="position:static">Ctrl K</span></dt><dd>caută orice, sari la o secțiune, rulează o acțiune</dd><dt>Esc</dt><dd>închide panoul lateral</dd><dt>← →</dt><dd>în panoul lateral: apelul / clientul / proiectul anterior sau următor din lista afișată</dd><dt>Enter · Spațiu</dt><dd>deschide rândul, bara sau cardul pe care e focusul (Tab)</dd><dt>Back (telefon)</dt><dd>închide panoul, nu aplicația</dd><dt>Instalare</dt><dd>din meniul browserului „Instalează aplicația” — merge și offline</dd></dl></div>';
